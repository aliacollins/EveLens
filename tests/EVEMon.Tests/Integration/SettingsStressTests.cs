...
namespace EVEMon.Tests.Integration
{
    ...
    public class SettingsStressTests : IDisposable
    {
        ...
        // =====================================================================
        // SCENARIO 3: API Query Scheduling with 100 characters
        // =====================================================================

        #region Query Scheduling at Scale

        [Fact]
        public void QueryScheduler_100Characters_AllRegistered()
        {
            ...
            using var scheduler = new SmartQueryScheduler(dispatcher, esiClient, random);

            for (int i = 0; i < 100; i++)
            {
                var q = Substitute.For<IScheduledQueryable>();
                q.CharacterID.Returns((long)(i + 1));
                q.IsStartupComplete.Returns(true);
                q.ConsecutiveNotModifiedCount.Returns(0);
                scheduler.Register(q);
            }

            scheduler.RegisteredCount.Should().Be(100);
        }

        [Fact]
        public void QueryScheduler_100Characters_StaggeredStartup_OnlyOneBackgroundPerTick()
        {
            var esiClient = Substitute.For<IEsiClient>();
            esiClient.MaxConcurrentRequests.Returns(20);
            esiClient.ActiveRequests.Returns(0L);
            ...
            // Set character 1 as visible (gets processed every tick)
            scheduler.SetVisibleCharacter(1);

            // First tick
            scheduledCallback?.Invoke();

            // Count how many background characters got ProcessTick called
            int processed = queryables.Skip(1) // skip visible character
                .Count(q => q.ReceivedCalls().Any(c => c.GetMethodInfo().Name == "ProcessTick"));

            processed.Should().BeLessOrEqualTo(1,
                "round-robin should process at most 1 background character per tick");
        }

        [Fact]
        public void QueryScheduler_100Characters_VisibleCharacterAlwaysProcessed()
        {
            var esiClient = Substitute.For<IEsiClient>();
            esiClient.MaxConcurrentRequests.Returns(20);
            esiClient.ActiveRequests.Returns(0L);
            ...
            var visibleQueryable = Substitute.For<IScheduledQueryable>();
            visibleQueryable.CharacterID.Returns(1L);
            visibleQueryable.IsStartupComplete.Returns(true);
            visibleQueryable.ConsecutiveNotModifiedCount.Returns(0);
            scheduler.Register(visibleQueryable);

            // Register 99 more background characters
            for (int i = 1; i < 100; i++)
            {
                var q = Substitute.For<IScheduledQueryable>();
                q.CharacterID.Returns((long)(i + 1));
            ...
            // Visible character should be processed on every tick
            int visibleProcessCount = visibleQueryable.ReceivedCalls()
                .Count(c => c.GetMethodInfo().Name == "ProcessTick");
            visibleProcessCount.Should().Be(10,
                "visible character is processed on every tick regardless of background count");
        }

        [Fact]
        public void QueryScheduler_RateLimitPressure_PausesBackgroundQueries()
        {
            var esiClient = Substitute.For<IEsiClient>();
            esiClient.MaxConcurrentRequests.Returns(20);
            // Simulate rate limit pressure: 18/20 = 90% > 80% threshold
            esiClient.ActiveRequests.Returns(18L);
            ...
            var visibleQ = Substitute.For<IScheduledQueryable>();
            visibleQ.CharacterID.Returns(1L);
            visibleQ.IsStartupComplete.Returns(true);
            scheduler.Register(visibleQ);

            var backgroundQ = Substitute.For<IScheduledQueryable>();
            backgroundQ.CharacterID.Returns(2L);
            backgroundQ.IsStartupComplete.Returns(true);
            scheduler.Register(backgroundQ);

            scheduler.SetVisibleCharacter(1);

            scheduledCallback?.Invoke();

            // Visible character still processed
            visibleQ.ReceivedCalls()
                .Count(c => c.GetMethodInfo().Name == "ProcessTick")
                .Should().Be(1, "visible character is always processed");

            // Background character skipped due to rate limit pressure
            backgroundQ.ReceivedCalls()
                .Count(c => c.GetMethodInfo().Name == "ProcessTick")
                .Should().Be(0, "background queries paused during rate limit pressure");
        }

        #endregion

        // =====================================================================
        // SCENARIO 4: User closes EVEMon (graceful shutdown)
        // =====================================================================
