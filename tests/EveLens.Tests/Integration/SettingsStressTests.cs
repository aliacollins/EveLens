...
namespace EveLens.Tests.Integration
{
    ...
    public class SettingsStressTests : IDisposable
    {
        ...
        public SettingsStressTests()
        {
            ...
            _tempDir = Path.Combine(Path.GetTempPath(), "evelens-stress-" + Guid.NewGuid().ToString("N"));
        ...
        public async Task Coalescing_DispatcherPostCalled_ForExportOnUIThread()
        {
            ...
            // Export is called directly per save (no dispatcher for cross-platform reliability)
            manager.ActualWriteCount.Should().Be(1,
                "Export() is called once per actual write, not per Save() call");
        }
        ...
        // =====================================================================
        // SCENARIO 3: User closes EveLens (graceful shutdown)
        // =====================================================================
        ...
        {
            // === Phase 1: "Running EveLens" — save 100 characters ===
            ...
            // === Phase 2: "Restart EveLens" — load from JSON ===
            var loaded = await SettingsFileManager.LoadToSerializableSettingsAsync();
        ...
        // =====================================================================
        // SCENARIO 6: EveLens crashes mid-save
        // =====================================================================
        ...
        {
            // === Phase 1: "EveLens running" — add characters, save gets coalesced ===
            var settings = BuildSettingsWith(100);
