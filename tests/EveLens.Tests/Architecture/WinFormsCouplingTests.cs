...
{
    /// <summary>
    /// Architecture tests enforcing WinForms decoupling from EveLens.Common service/model layer.
    /// Prevents regression — model and service files should use platform-agnostic interfaces.
    /// </summary>
    public class WinFormsCouplingTests
    {
        private static readonly Assembly CommonAssembly = typeof(EveLens.Common.Services.AppServices).Assembly;
        ...
        /// EveLens.Common.Threading.Dispatcher (WinForms-only).
        ...
        public void SSOServices_UseAppServicesDispatcher_NotStaticDispatcher()
        {
            ...
            foreach (var file in ssoFiles)
            {
                ...
                if (content.Contains("using EveLens.Common.Threading;"))
                    violations.Add($"{fileName}: still imports EveLens.Common.Threading");
            ...
            violations.Should().BeEmpty(
                ...
                "not EveLens.Common.Threading.Dispatcher.Invoke (WinForms-only). " +
                $"Violations: {string.Join("; ", violations)}");
        }
