...
**Architecture:** Dictionary-based `Loc` static class in `src/EveLens.Common/Services/LocalizationService.cs`. No .resx files — translations are inline dictionaries for simplicity with 2 languages.
...
3. Use `Loc.Get("key")` in code-behind (not AXAML — set via code-behind after InitializeComponent)

**Languages:** `en` (English), `zh-CN` (Simplified Chinese), `ko` (Korean). Language stored in `Settings.UI.Language`, applied on startup in `App.axaml.cs`. Changing language triggers auto-restart via `AppServices.ApplicationLifecycle.Restart()`. Adding a language: add a `Register<Lang>()` dictionary in `LocalizationService.cs` + entry in `AvailableLanguages`/`GetLanguageDisplayName`, and add the SDE code to `StaticTranslations.s_translatedLanguages` with a generated `eve-translations-<code>.xml.gzip`.

**SDE Translations:** `StaticTranslations` class loads 50K+ localized type/group names per language from `eve-translations-<code>.xml.gzip` (bundled: `zh-CN`, `ko`). Languages to load are listed in `StaticTranslations.s_translatedLanguages`. Generated by `tools/generate-translations.ps1 -Language <sde-code>` from CCP's Static Data Export (e.g. `-Language ko`, `-Language zh`). `StaticSkill.LocalizedName`, `StaticSkillGroup.LocalizedName`, `Item.LocalizedName`, `MarketGroup.LocalizedName` all check translations first, fall back to English `Name`.

**Key files:**
- `src/EveLens.Common/Services/LocalizationService.cs` — All UI string translations
- `src/EveLens.Common/Data/StaticTranslations.cs` — SDE game data translations
- `src/EveLens.Common/Resources/eve-translations-zh-CN.xml.gzip` — 50K type + 2.6K group translations
- `src/EveLens.Common/Resources/eve-translations-ko.xml.gzip` — 50K type + 1.5K group translations (Korean)
- `tools/generate-translations.ps1 -Language <code>` — Downloads SDE and generates a translation file for any SDE language (`zh`→zh-CN, `ko`, `ja`, `ru`, `fr`, `de`, `es`)
- `tools/parse-sde.ps1` — Parses SDE YAML for localized names
...
**Key files:**
- `src/EveLens.Common/SettingsObjects/GlobalPlanTemplateSettings.cs` — Data model
- `src/EveLens.Common/ViewModels/GlobalPlanDashboardViewModel.cs` — ViewModel with CRUD, comparison, plan generation
- `src/EveLens.Avalonia/Views/Dialogs/GlobalPlanDashboardWindow.axaml(.cs)` — UI
- `tests/EveLens.Tests/ViewModels/GlobalPlanDashboardViewModelTests.cs` — 17 tests
...
8. **Run full suite**: `dotnet test tests/EveLens.Tests/EveLens.Tests.csproj` — 1857+ must pass
...
22. **Portrait/Image Loading Is Async Code-Behind** — Character portraits and corporation logos load via `ImageService.GetCharacterImageAsync()` / `CorporationImage` in code-behind after visual tree attachment. `ImageService` returns SkiaSharp `SKBitmap` (cross-platform, NOT `System.Drawing` — GDI+ types throw `PlatformNotSupportedException` on Linux/macOS). Use `DrawingImageToAvaloniaConverter.Instance` to convert `SKBitmap` to `Avalonia.Media.Imaging.Bitmap` (the converter name is historical; it encodes the `SKBitmap` to a PNG stream and is one-way). Never block the UI thread.
...
- **Tests:** 2,140 passing
- **SDE:** Build 3470007, Aug 2026 (52,863 types; first build with SKINR component data — see `tools/SDEFiles/yaml_extracted/skinr*.yaml`)
