...
The biggest feature EveLens has ever shipped: a full 3D ship studio built on CCP's own open-source Trinity engine, rendering your SKINR designs -- and the entire Paragon Hub marketplace -- with the game's real assets, materials, and lighting.

- **The SKINR Hub** -- The SKINR viewer grew into a full studio: the 3D ship fills the window with the design's name, tier, and hull identity overlaid; a design carousel along the bottom shows real rendered thumbnails of your collection (captured as you browse, instant forever after); live search filters designs by name or hull; and an environment switcher offers five looks -- Studio, Hangar, Space, Sunlight, and Beauty -- applied without rebuilding the ship. Drag-rotation gained physical feel: flick the ship and it keeps spinning, gliding to a stop like it has mass -- grab it anytime to stop it. The Hub wears its own violet identity as EveLens's creative-marketplace space.
- **Ships light like the game lights them** -- The 3D preview gained the studio's missing fill light: hulls now read from every angle instead of falling to black off-sun, gold coatings keep their warmth at any zoom, and the Triglavian orb finally casts its red glow onto surrounding plating -- driven by each light's own authored position and colour, on every hull that carries one.
- **Paragon Hub discovery (first cut)** -- The Hub tab in the SKINR window now opens a live marketplace browser: the public Paragon Hub feed grouped one card per design, with the design's name, creator, cheapest PLEX ask, and how many listings are buyable right now. Search by name or creator, filter to a specific ship, and click any card to see that design rendered in full 3D on its hull -- owned or not, the public recipe renders exactly the same. The browse is ship-first: open the Hub while viewing a design and it starts on "Find a design for your <ship>"; opening a card keeps you inside the Hub with a Back-to-results button, and the Collection tab always returns to your own designs. Resolved design details are cached on disk, so the ship filter fills instantly from the second browse on. A market tree (ship class > faction > ship, with design counts) drives the browse like EVE's own market window, and a background renderer quietly fills every card with a real skinned render of that exact design -- same camera, same lighting, so designs are judged side by side rather than through mismatched screenshots. Until a design's render is ready its card shows the plain base hull, dimmed and labelled, so a stock photo can never be mistaken for the skin. Price history, recommendations and value labels are the next phases.
- **Photo op: assemble your fleet** -- The SKINR stage learned formation flying: a new Photo Op button lets you pick up to ten of your other designs as wingmen and a formation to assemble them into -- Vic, Line Abreast, Echelon, Column, or Wall -- with spacing computed from each hull's real measured size, so a shuttle and a battleship both sit right. Switching formations re-forms the fleet instantly, and every ship is freely placeable in 3D: Ctrl+drag slides it in the plane you're looking at, Ctrl+scroll pushes it deeper, orbit the camera and repeat -- the fleet goes exactly where you want it. The camera pulls back to frame the formation without giving up close zoom on a single hull (a photo op happens in open space; heading back to the hangar, or loading a new design, quietly disbands the escort). Your ships, your skins, one screenshot.
- **The EveLens Rendering Runtime is a separate add-on** -- The 3D engine no longer needs a developer build tree: EveLens now offers a one-time download of the EveLens Rendering Runtime on first SKINR use. The consent panel shows exactly what it is before you agree -- name, version, size, download host, and full SHA-256 -- and every package is verified against a signing key built into EveLens (signature, then every file's hash) before anything runs. Required only for 3D rendering; all of EveLens works without it. The SKINR studio also now carries CCP's trademark notice alongside the art it renders.
- **A real station hangar** -- The Hub's Hangar environment now parks your ship inside the actual Jita 4-4 Caldari Navy docking bay: the holo-lit landing pad, the traffic decks, the bay's own authored lighting -- which stays the bay's no matter which design you switch to (each hull's own light colours previously repainted the whole station). Every hull floats at a pad clearance scaled to its size, freighters included, and the camera skims the deck but never orbits below it. The bay breathes like the game's: depth fog fades distant decks and walls into a steel-grey haze, so the interior reads with real scale instead of rendering clinically sharp to the far wall. The Space environment cycles through eight real region nebulae -- click it again for the next sky, each lit by that region's own sun -- and switching environments never rebuilds the ship. The design details panel (hull dimensions from the game data, design attributes, local favorites, copy-to-clipboard sharing) rounds out the studio.
...
- **macOS is on the runway** -- EveLens itself has shipped for Apple Silicon all along; the SKINR 3D renderer's Metal build is now in active CI. The gate is server-driven: the day the Mac runtime publishes, this very release starts offering the install -- no app update required. Until then the SKINR window says so honestly, and everything else in it already works.
- **The render runtime is Authenticode-signed** -- Every executable and native module in the Windows runtime package now carries a code signature on top of EveLens's own manifest verification chain. Two locks, different doors.
