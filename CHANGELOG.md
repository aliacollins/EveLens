...
- **The Attribute Optimizer finds real remap boundaries** -- Auto-placed remaps split where the primary/secondary attribute PAIR changes, not just the primary: a plan of Memory/Perception then Memory/Intelligence then Perception/Willpower skills used to read as one Memory block with the only remap at the very first skill (Issue #122, thanks shawndibble). When you have fewer remaps than boundaries, the shortest segments now merge away so the boundaries that matter keep their remap. And when an attribute booster is active, the optimizer says so -- live attributes beat any legal remap until it expires, which used to read as "bad values" (thanks Dureiken).
