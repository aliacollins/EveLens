...
namespace EveLens.Core
{
    /// <summary>
    /// Lightweight, Core-level service locator that bridges the gap between the
    /// <c>EveLens.Core</c> assembly and service implementations in <c>EveLens.Common</c>.
    ...
    /// and for model/infrastructure code that cannot reference <c>EveLens.Common</c>.
    /// All other code should use <c>AppServices</c> (in <c>EveLens.Common</c>) or
    ...
    /// <list type="bullet">
    ///   <item><c>AppServices</c> (in <c>EveLens.Common</c>) owns and creates all service instances.</item>
    ///   <item><c>ServiceLocator</c> (in <c>EveLens.Core</c>) holds references to the same instances,
    ///     accessible from assemblies that cannot reference <c>EveLens.Common</c>.</item>
    ...
    public static class ServiceLocator
    {
        ...
        /// Powered by standalone <c>TraceService</c> in <c>EveLens.Common</c>.
        /// </summary>
        public static ITraceService TraceService { get; set; }

        /// <summary>
        /// Gets or sets the application directory/file path provider.
        /// Snapshotted from <c>EveLensClient</c> static paths at startup.
        /// </summary>
        ...
        /// Delegates to <c>EveLensClient.Characters</c> and <c>MonitoredCharacters</c>.
        /// </summary>
        ...
        /// Delegates to <c>Properties.Resources</c> in <c>EveLens.Common</c>.
        /// </summary>
