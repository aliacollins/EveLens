...
namespace EveLens.Common.IgbService
{
    ...
    /// the in-game browser to allow in-game interaction with EveLens.
    /// </summary>
    public class IgbServer
    {
            ...
            string requestPath)
        {
            ...
            using (StreamWriter sw = new StreamWriter(stream))
            {
                ...
                client.Write("Server: EveLens/1.0\n");
                ...
                {
                    client.Write($"eve.trustme: {BuildHostAndPort(headers["host"])}/::EveLens needs your character information.\n");
                }
        ...
        private void ProcessRequest(string request, string requestPath, IDictionary<string, string> headers, TextWriter sw)
        {
            ...
            {
                sw.WriteLine("The in-game browser do not trust EveLens.<br/>");
                sw.WriteLine("<a href=\"\" onclick=\"CCPEVE.requestTrust('{0}')\">Trust EveLens</a>.",
            ...
            // Character not listed in EveLens 
            if (AppServices.Characters.All(x => x.Name != characterName))
            {
                sw.WriteLine("Hello {0}, this character is not recognized by EveLens!", characterName);
                return;
            }
        ...
        private static void WriteDocumentHeader(TextWriter sw)
        {
            ...
            sw.WriteLine("    </style>");
            sw.WriteLine("    <title>EveLens</title>");
            sw.WriteLine("  </head>");
            sw.WriteLine("  <body>");
        }
