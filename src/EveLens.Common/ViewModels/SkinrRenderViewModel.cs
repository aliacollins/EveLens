...
namespace EveLens.Common.ViewModels
{
    ...
    public sealed class SkinrRenderViewModel : ViewModelBase
    {
            ...
            CancellationToken ct = default)
        {
            ...
            if (preset != SkinrEnvironmentPreset.Space && Fleet.WingmenCount > 0)
            {
        ...
        {
            // Re-clicking the design already on stage is a no-op, not a rebuild: the
            // engine build is the expensive step of this whole feature, and redoing
            // it to arrive at the identical ship spent seconds saying nothing.
            if (recipe?.Id != null && HasDesign &&
                _host?.LoadedSkinrId == recipe.Id)
                return;
        ...
        /// <summary>
        /// The photo-op fleet: wingman lifecycle, formations and free 3D placement.
        /// Split out per Law 2 along the same seam as <see cref="SkinrStageCamera"/> —
        /// see <see cref="SkinrFleetController"/>.
        /// </summary>
        public SkinrFleetController Fleet =>
            _fleet ??= new SkinrFleetController(() => _host, _resolver, Camera,
                settled => RequestRender(settled), Status);

        private SkinrFleetController? _fleet;
