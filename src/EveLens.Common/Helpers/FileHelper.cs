...
namespace EveLens.Common.Helpers
{
    public static class FileHelper
    {
        ...
        /// This method will take care of the readonly attributes, prompting the user to EveLens removing them.
        /// Finally, should a <see cref="UnauthorizedAccessException"/> occurs, EveLens will display a readable message for the end-user.
        /// </summary>
        ...
        public static void CopyOrWarnTheUser(string srcFileName, string destFileName)
        {
            ...
            while (true)
            {
                ...
                catch (UnauthorizedAccessException exc)
                {
                    ...
                    string message = exc.Message;
                    message += "\r\n\r\nEveLens failed to save to a file. You may have " +
                        "insufficient rights or a synchronization may be active. " +
                        "Choosing to abort will make EveLens quit.";
        ...
        /// <summary>
        /// Request from the use the permission for EveLens to remove the read-only attributes on its own files
        /// </summary>
        ...
        private static bool HasPermissionToRemoveReadOnlyAttributes
        {
            get
            {
                lock (s_removeReadOnlyAttributesLock)
                {
                    ...
                    string message = "EveLens detected that some of its files are read-only, " +
                                     "preventing it to save its datas.\r\n\r\n";
                    message +=
                        "Choosing YES will allow EveLens to remove the read-only attributes on its own files (only).\r\n";
                    message += "Choosing NO will force EveLens to continue without writing its files. " +
                               "This can cause unexpected behaviours.\r\n\r\n";
                    message += "Note : if you restart EveLens and it still encounters read-only files, " +
                               "you will be prompted again.";

                    // Display the message box
                    DialogChoice result = AppServices.DialogService.ShowMessage(message,
                        @"Allow EveLens to make its files writable",
                        DialogButtons.YesNo,
                        DialogIcon.Error);
