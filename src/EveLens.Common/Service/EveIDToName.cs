...
using StringIDInfo = EveLens.Common.Service.IDInformation<string, string>;
...
namespace EveLens.Common.Service
{
    public static class EveIDToName
    {
        ...
        /// <returns>The entity name, or EveLensConstants.UnknownText if it is being queried.</returns>
        public static string GetIDToName(long id, bool bypass = false)
        {
            return s_lookup.LookupID(id, bypass) ?? EveLensConstants.UnknownText;
        }
            ...
            lock (s_cacheList)
            {
                serial.Entities.AddRange(s_cacheList.Select(item =>
                    new SerializableEveIDToNameListItem
                    {
                        ...
                        Name = item.Value?.Value ?? EveLensConstants.UnknownText,
                    }));
            }
        ...
        private sealed class GenericIDToNameProvider : IDToObjectProvider<string, string>
        {
            ...
            protected override string Prefetch(long id)
            {
                ...
                if (id == 0L)
                    ...
                    name = EveLensConstants.UnknownText;
