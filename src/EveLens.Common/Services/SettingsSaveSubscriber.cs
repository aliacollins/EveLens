...
namespace EveLens.Common.Services
{
    ...
    /// Extracted from EveLensClient.OnXxx() methods to decouple Models from Settings.Save().
    /// </summary>
    /// <remarks>
    /// Previously, Model files had to call EveLensClient.OnXxx() which bundled:
    ///   1. Trace() logging
    ///   2. Settings.Save()
    ///   3. EventAggregator.Publish()
    ///
    /// With this subscriber, Models can publish events directly and Settings.Save()
    /// is handled here automatically. The OnXxx() methods in EveLensClient.Events.cs
    ...
    public sealed class SettingsSaveSubscriber : IDisposable
    {
        ...
        public SettingsSaveSubscriber(IEventAggregator aggregator)
        {
            ...
            // in EveLensClient.OnXxx() methods.
            _subscriptions = new[]
            {
