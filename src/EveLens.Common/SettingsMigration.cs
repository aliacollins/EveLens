...
namespace EveLens.Common
{
    ...
    {
        /// <summary>
        /// Gets whether a migration from another EveLens fork was detected on startup.
        ...
        // Our fork identifier - used to detect settings from other EveLens forks
        ...
        /// <summary>
        /// Detects if the settings file is from another EveLens fork.
        ...
        {
            string message;
            string title = "Welcome to EveLens 5.1";

            if (settingsCanBePreserved)
            {
                // Settings format is compatible - plans and settings will be preserved
                message = @"Welcome to EveLens 5.1!

It looks like you're coming from a different version of EveLens.

Due to how EVE's login system works, your characters need to be re-added. This is a one-time thing - ESI authentication tokens are tied to the specific EveLens version that created them and cannot be transferred.
...
Unfortunately, your settings file format is too old to migrate. EveLens will start with fresh settings.
