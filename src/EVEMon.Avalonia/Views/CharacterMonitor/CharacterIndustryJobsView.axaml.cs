...
namespace EVEMon.Avalonia.Views.CharacterMonitor
{
    public partial class CharacterIndustryJobsView : UserControl
    {
        ...
        protected override void OnAttachedToVisualTree(VisualTreeAttachmentEventArgs e)
        {
            ...
            _dataUpdatedSub ??= AppServices.EventAggregator?.Subscribe<IndustryJobsUpdatedEvent>(OnDataUpdated);
            LoadData();
        }
        ...
        private void LoadData()
        {
            ...
            if (oc != null && !oc.IsEndpointEnabled(ESIAPICharacterMethods.IndustryJobs))
            {
            ...
            if (oc != null && !oc.HasScopeFor(ESIAPICharacterMethods.IndustryJobs))
            {
        ...
        private void OnEnableEndpoint(object? sender, RoutedEventArgs e)
        {
            var parentView = this.FindAncestorOfType<CharacterMonitorView>();
            var oc = parentView?.DataContext as ObservableCharacter;
            oc?.EnableEndpoint(ESIAPICharacterMethods.IndustryJobs);
            LoadData();
        }

        private void OnDataUpdated(IndustryJobsUpdatedEvent evt)
        {
