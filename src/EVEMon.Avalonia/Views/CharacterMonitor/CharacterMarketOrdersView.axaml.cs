...
{
    public partial class CharacterMarketOrdersView : UserControl
    {
        ...
        protected override void OnAttachedToVisualTree(VisualTreeAttachmentEventArgs e)
        {
            ...
            _dataUpdatedSub ??= AppServices.EventAggregator?.Subscribe<CharacterMarketOrdersUpdatedEvent>(OnDataUpdated);
            LoadData();
        }
        ...
        private void LoadData()
        {
            ...
            if (oc != null && !oc.IsEndpointEnabled(ESIAPICharacterMethods.MarketOrders))
            {
            ...
            if (oc != null && !oc.HasScopeFor(ESIAPICharacterMethods.MarketOrders))
            {
        ...
        private void OnEnableEndpoint(object? sender, RoutedEventArgs e)
        {
            var parentView = this.FindAncestorOfType<CharacterMonitorView>();
            var oc = parentView?.DataContext as ObservableCharacter;
            oc?.EnableEndpoint(ESIAPICharacterMethods.MarketOrders);
            LoadData();
        }

        private void OnDataUpdated(CharacterMarketOrdersUpdatedEvent evt)
        {
