...
namespace EVEMon.Core.Interfaces
{
    ...
    /// Replaces the old 74 static events on <c>EveMonClient</c> with a decoupled pub/sub pattern
    ...
    ///
    /// Production: <c>EventAggregator</c> in <c>EVEMon.Infrastructure/Services/EventAggregator.cs</c>
    /// (also aliased from <c>EVEMon.Common/Services/</c> via <c>AppServices.EventAggregator</c>).
