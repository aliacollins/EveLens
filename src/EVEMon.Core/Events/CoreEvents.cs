...
{
    /// <summary>
    /// Base class for character-scoped events bridged from <c>EveMonClient</c>.
    /// Uses only primitive types (<c>long</c>, <c>string</c>) so that the Core assembly
    /// does not depend on the <c>Character</c> model in <c>EVEMon.Common</c>.
    /// </summary>
    /// <remarks>
    ...
    ///
    /// These are published by <c>EveMonClientCharacterServices</c> (in
    /// <c>EVEMon.Common/Services/</c>) when the corresponding <c>QueryMonitor</c> completes.
    /// They are the Core-layer equivalents of the typed events in
    /// <c>EVEMon.Common.Events</c> (which carry the full <c>Character</c> reference).
    /// </remarks>
