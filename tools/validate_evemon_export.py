#!/usr/bin/env python3
"""
EveLens Export Validator

This script reads the CSV exported from EveLens (Ctrl+Shift+D in Plan Window)
...
@dataclass
class SkillEntry:
    """Parsed skill entry from EveLens export"""
    name: str
    level: int
    rank: int
    primary_attr: str
...
def validate_entry(entry: SkillEntry, tolerance: float = 0.01) -> List[str]:
    ...
    if sp_hour_diff > 1:  # Allow 1 SP/hour rounding difference
        errors.append(f"SP/hour mismatch: EveLens={entry.sp_per_hour_omega:.0f}, Expected={expected_sp_hour:.0f}")
    ...
    if time_diff > tolerance:
        errors.append(f"Training time mismatch: EveLens={entry.training_time_hours:.4f}h, "
    ...
    if entry.old_training_time_hours > 0 and saved_diff > tolerance:
        errors.append(f"Time saved mismatch: EveLens={entry.time_saved_hours:.4f}h, "
...
def parse_csv(filepath: str) -> Tuple[List[SkillEntry], dict]:
    """Parse EveLens validation CSV export"""
    entries = []
    metadata = {}
...
def main():
    if len(sys.argv) < 2:
        ...
        print("\nTo export from EveLens: Press Ctrl+Shift+D in the Plan Window")
        sys.exit(1)
    ...
    for entry in entries:
        ...
        print(f"{entry.name:<30} {entry.level:<4} {entry.sp_per_hour_omega:>7.0f} {entry.training_rate:>5.1f} "
              f"{format_time(entry.training_time_hours):>12} {old_time_str:>12} {saved_str:>10} {status:<8}")
